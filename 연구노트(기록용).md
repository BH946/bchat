...
- 채팅 내용 기록위해 매번 채팅 DB INSERT 트랜잭션 에서 -> 배치 방식 변경. 일정량 쌓이거나 일정 시간 지나면 "배치처리" 등등 다양한 방법 사용
  => 졸작에서 했던거 참고
- validateDuplicateUser 함수에 find쿼리문 2개 튜닝...
  => 두 로직 합치기
- 메시지 이전 대화 출력(전체. 튜닝? 전체->부분? 파티셔닝? 등) 
  => 파티셔닝 쓰는지 알아보고 레고레고 -> 시간별 파티셔닝을 사용해 하루 단위로 메시지를 저장하거나, 사용자별 파티셔닝을 통해 유저 ID에 따라 데이터를 분할
  => 우리는 유져ID에 따라 파티셔닝 ㄱ
...
- 도메,레포,서비,컨로는 **"정리했던.md에" ㄱㄱㄱ** -> 뭔말인지 모르겠어서 PASS
...
1.(라이브러리추가)외부 의존성 추가 주의 및 추가된 것 확인은 external libraries 하위 참고 -> **OK**
