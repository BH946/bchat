...
**멀티스레드** -> 아예 서버를 따로 뺀게 아니다보니 종종 사용하게 되고 있다.
...
- 채팅방 내부 "이전 대화 출력" 기능 부분 -> 우선 DB내용 전체 출력
  - **이때, 이를 요청한 클라한테만 출력해주기.** -> 서버쪽은... 굳이 출력까진..? 안해도..?
  - 당연히 **실시간 대화 내용은 전체 클라한테 출력해주기**.
  - DB는?? ChatRoom, Message 추가 -> 일부러 테이블 수를 줄임.
    - DB엔 방장이 가진 ChatRoom들이 있을텐데 GUI에서 "제목"리스트를 설정 받아 "이전 대화 기능" 활성화
    - 아니 근데 애초에 그냥 "제목"으로 구분지어서 "이전대화출력" 바로 하면 되겠는디? ㅇㅇ
      - 제목은 유니크 키로
    - 메시지는 **userId+content**를 넘기고, 채팅방은 **userId+title** 있으면 충분
      - **DB 기록은 서버 단에서만 한다.**

일단 GUI 없이 테스트 -> 에러 수정 (에러 파티다 지금.) + 클라 전체한테 출력해주기는 아직 안했음.
